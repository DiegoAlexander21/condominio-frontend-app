...
export class FinanzasService {
  ...
  listarGastos(tipo?: string, page: number = 0, size: number = 10): Observable<PaginaResponse<GastoResponse>> {
    ...
    return this.http.get<PaginaResponse<GastoResponse>>(`${this.url}/gastos`, { params });
  }
  ...
  verMiEstadoCuenta(): Observable<any> {
    return this.http.get<any>(`${this.url}/estados-cuenta/mis-estados`);
  }
  ...
  listarPagos(estado?: string, page: number = 0, size: number = 10): Observable<PaginaResponse<PagoResponse>> {
    let params = new HttpParams()
      .set('page', page.toString())
      .set('size', size.toString());
    ...
    return this.http.get<PaginaResponse<PagoResponse>>(`${this.url}/pagos`, { params });
  }

  aprobarPago(id: number, formulario: { aprobar: boolean; observacionAdmin?: string }): Observable<PagoResponse> {
